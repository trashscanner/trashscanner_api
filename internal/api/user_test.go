...
func TestSoftAuthMiddleware(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		server, _, authMock, _, _ := newTestServer(t)

		token := "access.token"
		claims := &auth.Claims{UserID: testdata.User1.ID.String(), Login: testdata.User1.Login, Role: string(models.RoleAdmin)}
		...
		nextCalled := false
		handler := server.softAuthMiddleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			nextCalled = true
			user := utils.GetUser(r.Context())
			require.NotNil(t, user)
			assert.Equal(t, claims.UserID, user.ID.String())
	...
	t.Run("invalid token", func(t *testing.T) {
		...
		handler := server.softAuthMiddleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			nextCalled = true
			val := r.Context().Value(utils.UserCtxKey)
			assert.Nil(t, val)
		}))
...
func TestRoleMiddleware(t *testing.T) {
	t.Run("access allowed", func(t *testing.T) {
		...
		nextCalled := false
		handler := server.roleMiddleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			nextCalled = true
		}))

		req := httptest.NewRequest(http.MethodGet, "/api/v1/health", nil) // /api/v1/health is allowed for anonymous, user, admin in test config
		req = req.WithContext(utils.SetUser(req.Context(), &user))
	...
	t.Run("access denied", func(t *testing.T) {
		...
		// Set rules to deny anonymous
		server.authConfig.Rules = []config.AuthRule{
			{Pattern: "/**", Roles: []string{"admin"}}, // Only admin can access
		}

		user := testdata.User1
		user.Role = models.RoleUser // User does not have admin role
	...
	t.Run("anonymous access allowed", func(t *testing.T) {
		server, _, _, _, _ := newTestServer(t)

		server.authConfig.Rules = []config.AuthRule{
			{Pattern: "/public", Roles: []string{"anonymous"}},
		}
